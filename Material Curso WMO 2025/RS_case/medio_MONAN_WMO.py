...
output_dir = "figuras/rodada_2025050200/medio"
...
for dia in range(dia_inicial, dia_final + 1):
    ...
    for hora in range(hora_inicial, hora_final + 1):
        ...
        u500 = ds.uvel_isobaric.sel(level=50000)[0, :, :]
        v500 = ds.vvel_isobaric.sel(level=50000)[0, :, :]
    
        #ur_500 = ds.r_isobaric.sel(level=50000)[0, :, :]
    
        geo_pot500 = ds.zgeo_isobaric.sel(level=50000)[0, :, :]
    
        omeg_500 = ds.ommt_isobaric.sel(level=50000)[0, :, :]
        ...
        #ur500 = ax.contourf(lons, lats, ur_500, levels = np.arange(40, 101, 5),  cmap = 'Greens' , extend = 'neither', transform=ccrs.PlateCarree())
        ...
        #cbar = plt.colorbar(ur500, cax=axins, orientation='horizontal')
    
        # Definindo os marcadores da barra de cores para inteiros usando FixedLocator
        #cbar.ax.xaxis.set_major_locator(ticker.FixedLocator(np.arange(40, 101, 10)))
        ...
        geo_pot500 = ax.contour(lons, lats, geo_pot500/10, levels = np.arange(510, 640, 4), linewidths=1.0, colors = 'darkblue', transform=ccrs.PlateCarree())
        ax.clabel(geo_pot500, inline = True)
    
        #ax.streamplot(lons, lats, u500, v500, color='k', linewidth=0.5, density= 2, transform=ccrs.PlateCarree())
        
        plot_barbs_auto(ax, lons, lats, u500.values, v500.values, max_points=20, length=5.5, flip_barb=True, zorder=4,color='k')
    
        omeg_500 = ax.contour(lons, lats, omeg_500, levels = np.arange(-2.4, -0.1, 0.4),colors = 'red', linewidths= 0.5, transform=ccrs.PlateCarree())
        ax.clabel(omeg_500, inline = False)
        ...
        ax.text(0.5, 0.03, f'Vento, Geopotencial e Ômega em 500hPa - MONAN/INPE - Data Inicialiazação: 2025-05-02 00Z - Validade:{data_hora.strftime("%Y-%m-%d %H")}Z ', transform=ax.transAxes, fontsize=8, ha='center', va='top', fontweight='bold', bbox=dict(facecolor='white', alpha=0.8, edgecolor='black', boxstyle='round,pad=0.3'))
        ...
        #plt.savefig('/dados/nowcasting/maicon.veber/caso_RS/rodada_2025050200/medio/medio_MONAN_{:03d}.png'.format(contador), dpi=100, bbox_inches='tight')
        plt.savefig(output_dir + '/medio_MONAN_{:03d}.png'.format(contador), dpi=100, bbox_inches='tight')
