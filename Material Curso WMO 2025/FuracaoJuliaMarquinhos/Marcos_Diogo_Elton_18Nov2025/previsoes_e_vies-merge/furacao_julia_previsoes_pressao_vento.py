...
for i, rodada in enumerate(rodadas):
    for j, previsao in enumerate(previsoes):
        if rodada == previsao:  # Ignora casos onde rodada e previsão são a mesma data (sem dados).
            continue
        pasta = rodada  # Pasta onde o arquivo está armazenado.
        arquivo = f"MONAN_DIAG_R_POS_GFS_{rodada}_{previsao}.00.00.x1.5898242L55.nc"  # Nome do arquivo NetCDF.
        caminho = ('/pesq/share/monan/curso_OMM_INPE_2025/Central_America_Hurricane_Julia/' + pasta + '/' + arquivo)
...
for i, rodada in enumerate(rodadas):
    for j, previsao in enumerate(previsoes):
        ...
        try:
            ...
            caminho_atual = '/pesq/share/monan/curso_OMM_INPE_2025/Central_America_Hurricane_Julia/' + pasta + '/' + arquivo_atual
...
plt.suptitle("Furacão Julia (2022). Velocidade e direção do vento (10 m) e Pressão ao Nível Médio do Mar (hPa)", fontsize=15, y=0.95)
plt.savefig("furacao_julia_previsao_pressao_vento.png", dpi=300, bbox_inches='tight')  # Salva a imagem.
plt.close()  # Fecha a figura para liberar memória.
print("✅ Figura salva como furacao_julia_previsao_pressao_vento.png")
