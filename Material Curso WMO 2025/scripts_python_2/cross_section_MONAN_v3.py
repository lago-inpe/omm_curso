...
def processa_datos_monan(path, exp = 1, smooth_flag = 1):
	...
	ds = xr.open_dataset(path + '#mode=bytes').metpy.parse_cf()
...
def main():
    ...
    if len(sys.argv) > 1:
        print(f"\nLeyendo la lista de archivos NetCDF disponibles en:\n\n <{sys.argv[1]}>\n")
        url = sys.argv[1]
        result = get_url_paths(url, ext)

        for index, value in enumerate(result):
            print(f"Archivo: {value}, Índice: {index}")
        try:
            index = 0
            while index >= 0:
                user_input_str = input("\nIngrese el índice del archivo deseado para realizar el experimento, o un valor < 0 para salir: ")
                index = int(user_input_str)
                if index >= 0:
                    exp, smooth_flag = map(int, input("\nIngrese com exp (num experimento???) y smooth_flag (???): ").split())
                    print(f"\nProcesando el archivo {result[index]} ...")
                    processa_datos_monan(result[index], exp, smooth_flag)
                else:
                    print(" Cierre del programa ...\n")

        except ValueError:
            print("\nError al escribir. El índice debe ser un valor entero.\n")

    else:
        print("\nProporcione la URL que contiene los archivos MONAN como argumento para ejecutar el experimento. Ejemplo: python cross_section_MONAN_v3.py url_arg")
        print("url_arg puede ser:  https://dataserver.cptec.inpe.br/dataserver_dimnt/monan/curso_OMM_INPE_2025/MendozaAR_ZondaWind_3km_1h/2023071900/\n")
