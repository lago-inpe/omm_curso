...
rodadas = ["2024042700", "2024042800", "2024042900", "2024043000", "2024050100", "2024050200"]
previsoes = ["2024042800", "2024042900", "2024043000", "2024050100", "2024050200", "2024050300"]

# Limites geográficos do mapa: longitude (oeste a leste) e latitude (sul a norte).
lon_min, lon_max = -70, -40
lat_min, lat_max = -45, -20
#extent = [-70, -40, -45, -20]
...
for i, rodada in enumerate(rodadas):
    for j, previsao in enumerate(previsoes):
        if rodada == previsao:  # Ignora casos onde rodada e previsão são a mesma data (sem dados).
            continue
        pasta = ('/pesq/share/monan/curso_OMM_INPE_2025/caso_RS/' + rodada + '/')  # Pasta onde o arquivo está armazenado.
        #/pesq/share/monan/curso_OMM_INPE_2025/caso_RS/2024042700_2024050300/MONAN_DIAG_R_POS_GFS_2024042700_2024042700.mm.x20.835586L55.nc
...
for i, rodada in enumerate(rodadas):
    for j, previsao in enumerate(previsoes):
        ...
        try:
            ...
            pasta = ('/pesq/share/monan/curso_OMM_INPE_2025/caso_RS/' + rodada + '/')
...
plt.savefig("previsao_pressao_vento_rs.png", dpi=300, bbox_inches='tight')  # Salva a imagem.
